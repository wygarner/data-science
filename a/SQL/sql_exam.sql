...
SELECT state, COUNT(avg_math_4_score) AS score_count, ROUND(AVG(avg_math_4_score),2) AS avg_score,
MIN(avg_math_4_score) AS min_score, MAX(avg_math_4_score) AS max_score
FROM naep
GROUP BY state
ORDER BY state;
...
-- 5. Write a query that returns a field called bottom_10_states that lists the states in 
--    the bottom 10 for avg_math_4_score in the year 2000.
...
SELECT ROUND(AVG(avg_math_4_score),2) as avg_score_y2000
FROM naep
WHERE year='2000';
...
-- 8. Write a query that returns a field called scores_missing_y2000 that lists any states with 
...
--    by total_expenditure greatest to least. Be sure to round avg_math_4_score to the nearest 2 decimal places, 
--    and then filter out NULL avg_math_4_scores in order to see any correlation more clearly.
SELECT naep.state, ROUND(naep.avg_math_4_score,2), finance.total_expenditure
